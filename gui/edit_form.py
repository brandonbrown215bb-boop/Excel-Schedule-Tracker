...
class EditForm(QWidget):
    ...
    def _on_save(self):
        ...
        # Build a new Unit from form values
        updated = Unit(
            com_number=self.com_number_edit.text(),
            job_name=self.job_name_edit.text(),
            contract_number=self.contract_edit.text(),
            description=self.description_edit.text(),
            detailer=self.detailer_edit.currentText(), # Get current text from QComboBox
        ...
        self.status_label.setText("<span style='color: green;'>✓ Saved — refresh to see updates</span>")
        self._dirty = False
        self.dirty_changed.emit(False)
